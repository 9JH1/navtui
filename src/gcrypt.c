int main(void){
	...
	auth_config settings = {
		...
		.password = "bongo.cat",
		.server_url = "http://192.168.68.108:4533",
	};

	auth_subsonic(&settings);

	char *url = get_full_url(&settings,"/rest/getPlaylists");
	char *response = fetch_url_content(url);
	printf("%s\n",response);
